...
mock.onGet('/home/crypto').reply(200, [
	{ label: 'Bitcoin', oldPrice: 56915.63, change: getRandom(3000), changePercent: '1.59%' },
	{ label: 'Ethereum', oldPrice: 1811.21, change: getRandom(180), changePercent: '0.97%' },
	{ label: 'Binance', oldPrice: 282.29, change: getRandom(30), changePercent: '2.66%' }
].map(item => ({
	...item,
	price: (item.oldPrice + item.change).toFixed(2),
	changePercent: `${((item.change) / item.oldPrice * 100).toFixed(2)}%`,
