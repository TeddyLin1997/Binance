...
const hotCrypto: React.FC = () => {
	...
	const hotCryptoNode = hotList.map(item => {
		...
		return (
			<article className="px-2 py-1 w-28 h-20 flex flex-col justify-between bg-white rounded-lg font-bold text-center text-xs" key={item.label}>
				<span className=" text-secondary">{ item.label }</span>
				<div className="flex justify-between items-end">
					<span className="text-gray transform scale-90">價格</span>
					<span className={ color + ' text-sm' }>{ item.price }</span>
				</div>
				<div className="flex justify-between items-end">
